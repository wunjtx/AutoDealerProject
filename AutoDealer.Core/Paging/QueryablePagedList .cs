...
namespace AutoDealer.Core.Paging
{
    /// <summary>
    ...
    /// <seealso cref="List{T}"/>
    //[Serializable]        
    public class QueryablePagedList<T, TKey> : PagedList<T>
    {
        /// <summary>
        ...
        public QueryablePagedList(IQueryable<T> superset, Expression<Func<T, TKey>> keySelector, int pageNumber, int pageSize)
        {
            if (pageNumber < 1)
        ...
        public QueryablePagedList(PagedListMetaData pagedListMetaData, IEnumerable<T> superset)
        {
